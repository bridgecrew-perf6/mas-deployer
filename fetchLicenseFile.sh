...
echo "Loading parameters from license.properties"
...
curl 'https://licensing.subscribenet.com/control/ibmr/login' \
  ...
  --data-raw 'nextURL=%2Fcontrol%2Fibmr%2Fibmrindex&action=authenticate&username=$(urlencode ${username})&password=$(urlencode ${password})' \
  --compressed \
  --location \
  --cookie-jar /tmp/tokens.cookies \
  -o /tmp/output-step2.html

newUrl=$(cat /tmp/output-step2.html | grep "IBM AppPoint Suites" | sed 's/^.*href="\(.*\)".*$/\1/g')
