...
if [[ "$(oc get Kafka ${kafkaclustername} -n ${kafkanamespace})" == *"NotFound"* ]]; then
...
operator_name=$(oc get ClusterServiceVersion strimzi-cluster-operator.v0.22.1 -n ${kafkanamespace} -o jsonpath="{.spec.install.spec.deployments[0].name}")
...
while [[ $(oc get Subscription strimzi-kafka-operator -n ${kafkanamespace} --ignore-not-found=true -o jsonpath='{.status.state}') != "UpgradePending" ]];do sleep 5; done & 
showWorking $!
printf '\b'

echo "	Approving manual installation"
# Find install plan
installplan=$(oc get subscription strimzi-kafka-operator -o jsonpath="{.status.installplan.name}" -n ${kafkanamespace})
echo "	installplan: $installplan"

# Approve install plan
oc patch installplan ${installplan} -n ${kafkanamespace} --type merge --patch '{"spec":{"approved":true}}' > /dev/null 2>&1

while [[ $(oc get ClusterServiceVersion -n ${kafkanamespace} --no-headers | grep strimzi-cluster-operator | awk '{printf $1}') == "" ]];do sleep 1; done & showWorking $!
printf '\b'

operator_name=$(oc get ClusterServiceVersion strimzi-cluster-operator.v0.22.1 -n ${namekafkanamespacespace} -o jsonpath="{.spec.install.spec.deployments[0].name}")


echo -n "	Operator ready              "
while [[ $(oc get deployment/${operator_name} --ignore-not-found=true -o jsonpath='{.status.readyReplicas}' -n ${kafkanamespace}) != "1" ]];do sleep 5; done & 
showWorking $!
printf '\b'
echo -e "${COLOR_GREEN}[OK]${COLOR_RESET}"
...
echo -n "	Kafka ready              "
while [[ $(oc get Kafka ${kafkaclustername} --ignore-not-found=true -o jsonpath="{.status.conditions[?(@.type=='Ready')].status}" -n ${kafkanamespace}) != "True" ]];do sleep 5; done & 
showWorking $!
printf '\b'
echo -e "${COLOR_GREEN}[OK]${COLOR_RESET}"
...
spec:
  ...
  authorization:
    ...
    type: simple
EOF
oc apply -f tmp/kafka_user.yaml > /dev/null 2>&1
echo -e "${COLOR_GREEN}[OK]${COLOR_RESET}"

echo_h2 "[3/4] Creating topics..."
cat << EOF > tmp/kafka_topics.yaml
apiVersion: kafka.strimzi.io/v1beta2
kind: KafkaTopic
metadata:
  labels:
...
oc apply -f tmp/kafka_topics.yaml > /dev/null 2>&1
echo -e "${COLOR_GREEN}[OK]${COLOR_RESET}"
